...
/** Phase 5.1 (F-02) — persist a FAILED sign-in for a known user (count + timestamp + optional lock). */
export function recordFailedLogin(
  id: string,
  data: { failedLoginCount: number; lastFailedLoginAt: Date; lockedUntil: Date | null },
) {
  return prisma.user.update({ where: { id }, data });
}

/** Phase 5.1 (F-02) — reset the lockout state on a SUCCESSFUL sign-in. */
export function recordSuccessfulLogin(id: string, at: Date) {
  return prisma.user.update({
    where: { id },
    data: { failedLoginCount: 0, lockedUntil: null, lastSuccessfulLoginAt: at },
  });
}
