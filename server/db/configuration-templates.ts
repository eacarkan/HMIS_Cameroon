...
export async function applyTemplateToHospital(params: {
  ...
  return prisma.$transaction(async (tx) => {
    // Existing keys for create-vs-update counting (scoped to this hospital).
    const [existingDepts, existingServices, existingSettings, existingDocs] = await Promise.all([
      tx.department.findMany({ where: { hospitalId }, select: { code: true } }),
      tx.serviceUnit.findMany({ where: { hospitalId }, select: { code: true } }),
      tx.setting.findMany({ where: { hospitalId }, select: { key: true } }),
      tx.documentTemplate.findMany({ where: { hospitalId }, select: { type: true, name: true } }),
    ]);
