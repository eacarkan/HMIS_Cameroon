...
export async function startDiagnosticTx(params: {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (params.emergencyDebt) {
      ...
      if (!locked.isPaid) {
        const { debt, created } = await accrueEmergencyDebtWithinTx(tx, params.emergencyDebt, { idempotentBySource: true });
        return { emergencyDeferred: true, emergencyDebt: { id: debt.id, created } };
      }
    }
    return { emergencyDeferred: false, emergencyDebt: null as { id: string; created: boolean } | null };
  });
