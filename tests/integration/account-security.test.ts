...
describe("integration: Phase 1A Batch 4 — account security", () => {
  ...
  it("a user changes their own password (policy enforced); old fails, new works", async () => {
    ...
    await expect(changeOwnPassword(actor, getDemoSharedPassword(), "weak")).rejects.toThrow(/8 caractères/);

    await changeOwnPassword(actor, getDemoSharedPassword(), "Nouveau1234");
    expect(await authenticateCredentials(ACCOUNTS.cashier, getDemoSharedPassword())).toBeNull();
