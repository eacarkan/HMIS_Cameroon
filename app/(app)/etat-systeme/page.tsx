...
import { GATE7_DISCLAIMER_FR } from "@/lib/uat-gate7";
...
export default async function SystemStatusPage() {
  ...
  return (
    <>
      ...
      <div className="border-amber-300 bg-amber-50 text-amber-900 mb-3 rounded-md border px-4 py-2 text-sm font-semibold">
        {status.dataModeLabel}
      </div>
      {/* Phase 5H — pilot-readiness marker: evidence only, never an authorization; demo/synthetic data. */}
      <div className="border-primary/40 bg-primary/5 mb-6 rounded-md border px-4 py-2 text-xs">
        <strong>{GATE7_DISCLAIMER_FR}</strong>
        <span className="text-muted-foreground block">DÉMO — DONNÉES SYNTHÉTIQUES · {RELEASE_LABEL}</span>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            <Row label={t("releaseCandidate")} value={RELEASE_CANDIDATE} />
            <p className="text-muted-foreground text-xs">{RELEASE_LABEL}</p>
