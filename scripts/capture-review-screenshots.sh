...
login_token() {
  ...
  curl -s -b "$jar" -c "$jar" -X POST "http://localhost:$PORT/api/auth/callback/credentials" \
    ...
    --data-urlencode "password=${HMIS_DEMO_SHARED_PASSWORD:?set HMIS_DEMO_SHARED_PASSWORD (synthetic demo password) before capturing}" --data-urlencode "callbackUrl=http://localhost:$PORT/" -o /dev/null
